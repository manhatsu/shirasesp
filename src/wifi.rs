...
async fn connection_task(mut controller: WifiController<'static>) {
    ...
    defmt::info!("Connecting to SSID: {}", ssid);
        ...
        match sta_state() {
            WifiStaState::Started => {
                if let Err(e) = controller.connect_async().await {
                    defmt::error!("Failed to connect to Wi-Fi: {:?}", e);
                } else {
            ...
            WifiStaState::Disconnected => {
                defmt::warn!("Wi-Fi disconnected, retry connection");
                if let Err(e) = controller.connect_async().await {
                    defmt::error!("Failed to reconnect to Wi-Fi: {:?}", e);
                } else {
                    defmt::info!("Reconnected to Wifi");
                }
            }
            WifiStaState::Stopped => {
                defmt::warn!("Wi-Fi stopped, restarting connection");
                if let Err(e) = controller.start_async().await {
                    defmt::error!("Failed to restart Wi-Fi connection: {:?}", e);
                };
            }
            WifiStaState::Invalid => {
                defmt::error!("Invalid Wi-Fi state");
                if let Err(e) = controller.stop_async().await {
                    defmt::error!("Failed to stop Wi-Fi connection: {:?}", e);
                }
            }
            _ => {
                defmt::warn!("Unexpected Wi-Fi state: {:?}", sta_state());
            }
        }
